...
LOGDIR=backtest_results/
TIMESTAMP=`date "+%Y%m%d-%H%M%S"`
scriptstart=$SECONDS
SPOTCONFIGFILE=./config_backtest_usdt_spot_new.json
SPOTDATA=binance
...
do
    ...
    start=$SECONDS
    freqtrade backtesting --config $CONFIG --export trades --export-filename 'backtest_results/'$STRATEGY'-'$TPE'-'$TIMEFRAME --timeframe $TIMEFRAME --timerange=$TIMERANGE --strategy $STRATEGY --datadir data/$DATADIR 2>&1 | tee -a $LOGDIR/$STRATEGY-$TPE-$TIMESTAMP.$EXTENSION
    end=$SECONDS
...
echo Logs can be found in \"backtest_results\" and $LOGDIR/$STRATEGY-$TPE-$TIMESTAMP 2>&1 | tee -a $LOGDIR/$STRATEGY-$TPE-$TIMESTAMP.$EXTENSION
echo
echo To plot a profit chart of a backtest, use:
echo freqtrade plot-profit --config backtest-config.json --auto-open --timeframe [1d \| 4h \| 1h \| 30m \| 15m \| 5m]
echo
echo To plot a chart of a certain pair on a certain timeframe, use:
echo freqtrade plot-dataframe --config backtest-config.json --strategy $STRATEGY-$TPE-$TIMESTAMP --timeframe [1d \| 4h \| 1h \| 30m \| 15m \| 5m] --pair [best pair] [worst pair]
echo
